...
The generated knowledge approach was actually introduced for a completely different task, that of answering difficult questions. Consider the following question, which GPT-3 answers incorrectly:  <iframe
...
Let's start with the first step, *knowledge generation*. We can ask the %%LLM|LLM%% to generate facts about Congo and South Africa: <iframe
...
Next, let's use that knowledge to answer the question correctly. This is the *knowledge integration* step! <iframe
