---
sidebar_position: 90
---
# 🟢 Rekurzivní injekce

Jak jsme již viděli dříve, velmi dobrou obranou proti prompt hackingu je použití jednoho LLM k vyhodnocení výstupu jiného LLM, aby se zkontrolovalo, zda neexistuje nějaký protichůdný výstup. Tuto obranu je možné obejít pomocí útoku *rekurzivní injekce*. Tento útok spočívá v injektování promptu do prvního LLM, který vytvoří výstup, který obsahuje instrukci pro injekci druhého LLM.

Předpokládejme, že prompt, který se snažíme hacknout, je:

```text
Řekněte něco pozitivního o následujícím a povaze:
{{user_input}}
```

Výstup této výzvy bude vyhodnocen promptem:

```text
Obsahuje následující text nějakou vulgaritu nebo urážlivý obsah (ANO nebo NE)?
{{output}}
```

Oba tyto prompty můžeme hacknout připojením k promptu, jak je vidět níže. Všimněte si, že toto je také příklad útoku typu přiřazení proměnné(@kang2023exploiting).

<iframe
    src="https://embed.learnprompting.org/embed?config=%2BPiBwcmludChCLCAnXFxuJywgQywgJ1xcblxcbicsIEQsICdcXG4nLCBFLCBZLCBBLCBYKSIsIm1vZGVsIjoidGV4dC1kYXZpbmNpLTAwMyJ9"
    style={{width:"100%", height:"500px", border:"0", borderRadius:"4px", overflow:"hidden"}}
    sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"
></iframe>

Výše uvedený výstup je pokus o injekci, který se dostane na vstup druhého volání LLM:

<iframe
    src="https://embed.learnprompting.org/embed?config=%3D%3D"
    style={{width:"100%", height:"500px", border:"0", borderRadius:"4px", overflow:"hidden"}}
    sandbox="allow-forms allow-modals allow-popups allow-presentation allow-same-origin allow-scripts"
></iframe>

Rekurzivní injekce se provádějí obtížně, ale za správných okolností mohou být velmi užitečné.