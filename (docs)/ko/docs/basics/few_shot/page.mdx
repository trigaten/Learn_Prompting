export const metadata = { sidebar_position: 4, title: "🟢 퓨샷 프롬프팅" };
...
또 다른 프롬프트 전략은 _퓨샷 프롬프팅 (few shot prompting)_(@logan-iv-etal-2022-cutting)(@lake2015human)으로, 기본적으로 모델에 원하는 작업의 몇 가지 예시(보통 줄여서 shot이라고 함)를 제시합니다.
...
<Note>각 입력-출력 쌍은 *exemplar*라고 불립니다.</Note>
...
<Note>
  그냥 명령 프롬프트를 통해서도 같은 출력을 만들 수 있지만 퓨샷 프롬프트는 더
  *일관성*있는 출력을 제공할 것입니다.
</Note>
