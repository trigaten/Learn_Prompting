export const metadata = { sidebar_position: 7, title: "🟡 지식 생성하기" };
...
_지식 생성_ 단계를 시작해봅시다. 우리는 LLM에 콩고와 남아프리카에 대한 사실들을 생성하도록 질문할 수 있을 것입니다:
...
다음으로 지식을 이용해서 질문에 더 정확하게 답변할 수 있도록 만들어봅시다. 이것이 _지식 통합_ 단계입니다.
