...
<Note>Chaque paire entrée-sortie est appelée un _exemple_.</Note>
...
<Note>
  Même si vous pourriez produire cette même sortie en utilisant une instruction
  prompt à la place, le prompt few-shot aide à rendre la sortie beaucoup plus
  _cohérente_.
</Note>
