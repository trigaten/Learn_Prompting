export const metadata = { sidebar_position: 100, title: "🟢 Paramètres LLM" };
...
<Note>
  [pour les chercheurs] Ceux-ci sont différents des hyperparamètres réguliers
  tels que le taux d'apprentissage, le nombre de couches, la taille cachée, etc.
</Note>
...
<Note>
  Parfois, plus d'aléatoire peut être utile pour les tâches où l'exactitude est
  nécessaire lorsqu'il est associé à [des techniques de prompting
  spéciales](https://learnprompting.org/docs/intermediate/self_consistency).
</Note>
...
<Warning>
  Certains modèles, comme ChatGPT, ne vous permettent **pas** d'ajuster ces
