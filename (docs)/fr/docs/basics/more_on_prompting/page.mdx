...
```html
Twitter est une plate-forme de médias sociaux où les utilisateurs peuvent
publier de courts messages appelés "tweets". Les tweets peuvent être positifs ou
négatifs, et nous aimerions pouvoir classer les tweets comme positif ou négatif.
Voici quelques exemples de tweets positifs et négatifs. S'assurer pour classer
correctement le dernier tweet. Q : Tweet : "Quelle belle journée !" Ce tweet
est-il positif ou négatif ? A: positif Q : Tweet : "Je déteste ce cours" Ce
tweet est-il positif ou négatif ? A: négatif Q : Tweet : "J'adore les poches sur
les jeans" A:
```
