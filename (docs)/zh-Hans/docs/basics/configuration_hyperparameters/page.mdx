export const metadata = { sidebar_position: 100, title: "🟢 LLM Settings" };
...
<Note>
  [给研究者] 这些配置不同于学习率（learning rate）、层数（number of
  layers）及隐藏大小（hidden size）等通常的超参数。
</Note>
...
<Warning>有些模型，如 ChatGPT，**不允许**调整这些超参数配置。</Warning>
