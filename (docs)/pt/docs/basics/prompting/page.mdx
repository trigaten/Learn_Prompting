...
instruir uma IA a fazer uma tarefa é chamado de _prompting_(@shin2020autoprompt). Nós enviamos para a IA uma série de
instruções (o _prompt_) e ela executa a tarefa. _Prompts_ podem ser simples como uma pergunta, ou complexos como um
texto de vários parágrafos.

Abaixo, alguns exemplos de _prompts_:
...
<pre>
  <div className="bluegreen-highlight">
    Em 30 de setembro de 1985, uma chuva de granizo e chuvas intensas atingiu
    severamente as áreas dos vales do Rio Doce e Mucuri no leste de Minas
    Gerais, resultando em 20 vítimas fatais e 600 feridos em Itabirinha, o
...
pode criar um _prompt_ utilizando a pergunda "Quanto é \{\{EQUAÇÃO\}\}".

Para uma certa questão, seu _prompt_ completo pode ser algo como:

```txt
Quanto é 965*590?
```

Para esse _prompt_, a IA GPT-3 (text-davinci-003) algumas vezes responde incorretamente, por exemplo: `572850`. É aqui
onde a engenharia de _prompt_ entra em ação.
...
correta indicando a operação matemática a ser utilizada ajuda? Como nós podemos criar _prompts_ que geram ótimos
resultados em nossas tarefas? Essa última pergunta em particular, é o foco do campo de Engenharia de _Prompt_, bem como
desse curso.
...
Leia o conteúdo do curso para aprender como gerar bons _prompts_!
