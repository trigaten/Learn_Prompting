export const metadata = { sidebar_position: 3, title: "🟢 Prompt Debiasing" };
...
Depending on their distribution and order within the prompt, <Term term="exemplars"> exemplars </Term> may bias LLM outputs(@si2022prompting). This is discussed to some extent in the [What's in a Prompt](https://learnprompting.org/docs/intermediate/whats_in_a_prompt) page.
...
performing binary <Term term="sentiment analysis"> sentiment analysis </Term> (positive or negative) on tweets, and you
...
The order of <Term term="exemplars"> exemplars </Term> can also cause bias. For example, a prompt that has randomly ordered exemplars
